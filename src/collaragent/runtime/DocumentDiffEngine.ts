...
export class DocumentDiffEngine {
   ...
   */
  static computeDocumentDiff(current: DocumentPayload, target: DocumentPayload): EditorCommand[] {
    ...
    for (let i = 0; i < targetBlocks.length; i++) {
      ...
      const existingIdx = targetBlock.id
        ? currentBlocks.findIndex((b, idx) => idx > i && b.id === targetBlock.id)
        : -1
