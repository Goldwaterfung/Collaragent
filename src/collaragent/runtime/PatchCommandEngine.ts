...
export interface PatchCommandFailure {
  applied: false
  code: 'INVALID_PATCH' | 'PATCH_CONTEXT_MISMATCH'
  message: string
  hunkIndex?: number
  header?: string
}
...
export type PatchCommandEngineResult =
  ({ applied: true } & PatchCommandResult) | PatchCommandFailure
...
export class PatchCommandEngine {
   ...
   */
  static compile(currentContent: string, operations: PatchOperation[]): PatchCommandEngineResult {
    try {
      ...
      const existingIds = new Set(
        workingLines.map((line) => extractBlockId(line)).filter(Boolean) as string[]
      )
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        if (op.action === 'insert') {
          if (!op.anchor || !op.blockId) {
            throw new Error(
              `Operation ${i}: insert requires both blockId (anchorId) and anchor ('before'|'after').`
            )
          }
          if (!op.newHtml) {
