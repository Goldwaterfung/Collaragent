...
export const DEFAULT_READ_LINE_LIMIT = 1500;
...
/**
 * Zod v3 version of FileDataSchema for use in stateSchema (createMiddleware requires zod v3).
 */
const FileDataSchemaV3 = zv3.object({
  content: zv3.array(zv3.string()),
  created_at: zv3.string(),
  modified_at: zv3.string(),
});
 ...
 *
 * Uses zod v3 with a reducer metadata attached to _def, matching the pattern
 * used by createMiddleware in langchain.
 */
const _filesFieldSchema = zv3
  .record(zv3.string(), FileDataSchemaV3)
  .default({});

// Attach reducer metadata directly to the zod v3 schema's internal _def
// This is how LangGraph/langchain reads reducer configuration from stateSchema fields
(_filesFieldSchema as any)._def.metadata = {
  reducer: {
    fn: fileDataReducer,
    schema: zv3.record(zv3.string(), FileDataSchemaV3.nullable()),
  },
};

const FilesystemStateSchema = zv3.object({
  files: _filesFieldSchema,
});
...
Filesystem Tools are available to you and all subagents at all times.
