...
async function getFetch(): Promise<typeof fetch> {
  ...
  try {
    const nf: unknown = await import('node-fetch')
    if (
      typeof nf === 'object' &&
      nf !== null &&
      'default' in nf &&
      typeof nf.default === 'function'
    ) {
      return nf.default as typeof fetch
    }
    if (typeof nf === 'function') {
      return nf as typeof fetch
    }
    throw new Error('Invalid node-fetch module structure')
  } catch (e) {
    throw new Error('No fetch available in this runtime', { cause: e })
  }
}
...
export class CheckpointApiClient {
  ...
  async restoreCheckpointBundle(
    bundleId: string,
    options?: { sessionId?: string; threadId?: string }
  ): Promise<CheckpointBundle> {
    const payload = await this.request<{ bundle: CheckpointBundle }>('/checkpoints/restore', {
      method: 'POST',
