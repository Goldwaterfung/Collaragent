...
class WindowManager {
  ...
  private async requestFsProcess<T>(
    rec: WindowRecord,
    requestType: string,
    responseType: string,
  ...
  private async prepareClose(
    rec: WindowRecord,
    options: { saveToArchive: boolean }
  ): Promise<ProcessResult> {
    return await this.requestFsProcess<ProcessResult>(
      rec,
      'prepare-close',
      'close-prepared',
      options
  ...
  private async handleManagedWindowClose(rec: WindowRecord): Promise<void> {
    ...
    try {
      ...
      if (closeState.isArchiveBacked && closeState.isUpdated) {
        const sourceName = closeState.sourceArchivePath
          ? path.basename(closeState.sourceArchivePath)
          : path.basename(rec.filePath)
        const liveName = closeState.liveWorkspacePath
          ? path.basename(closeState.liveWorkspacePath)
          : `${path.basename(sourceName, '.cagent')}.collar`
  ...
  public async openFile(filePath: string, reuseWindow?: BrowserWindow): Promise<WindowRecord> {
    ...
    let fsPort: number
    try {
      fsPort = await new Promise<number>((resolve, reject) => {
        const timeout = setTimeout(() => {
          cleanup()
    ...
    const isManaged =
      reuseWindow && Array.from(this.windows.values()).some((r) => r.window.id === reuseWindow.id)

    if (reuseWindow && !isManaged && !reuseWindow.isDestroyed()) {
      console.log(
        `[WindowManager] Reusing existing window (id=${reuseWindow.id}) for file: ${path.basename(filePath)}`
      )
      win = reuseWindow
    } else {
  ...
  public async exportArchive(
    ...
    return await this.requestFsProcess<{ success: boolean; error?: string }>(
      rec,
      'export',
      'export-ready',
      { targetPath }
