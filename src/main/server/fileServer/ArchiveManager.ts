...
export class ArchiveManager {
  ...
  public async mount(sourceCagentPath: string, destDir?: string): Promise<string> {
    ...
    if (fs.existsSync(sourceCagentPath)) {
      ...
      console.log(
        `[ArchiveManager] Source file ${sourceCagentPath} does not exist. Created empty temp dir at ${tempDir}`
      )
    }
    ...
    return new Promise<void>((resolve, reject) => {
      output.on('close', async () => {
        try {
          ...
          console.log(
            `[ArchiveManager] Committed changes to ${destCagentPath} (${archive.pointer()} total bytes)`
          )
          resolve()
        } catch (err) {
          reject(err)
