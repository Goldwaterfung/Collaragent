...
export class FileCheckpointStore implements CheckpointStore {
  ...
  async putCheckpoint(record: CheckpointRecord): Promise<void> {
    const filePath = path.join(
      this.getCheckpointsDir(record.thread_id),
      `${record.checkpoint_id}.json`
    )
  ...
  async deleteBlobs(keys: string[]): Promise<void> {
    for (const key of keys) {
      ...
      if (fs.existsSync(filePath)) {
        await fs.promises
          .unlink(filePath)
          .catch((err) => console.error('Failed to delete blob', key, err))
      }
    }
  }
  ...
  async putWrites(threadId: string, writes: CheckpointWriteRecord[]): Promise<void> {
    for (const write of writes) {
      const filePath = path.join(
        this.getWritesDir(threadId),
        `${write.checkpoint_id}_${write.task_id}_${write.idx}.json`
      )
