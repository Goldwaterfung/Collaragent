...
export class CagentStorage extends EventEmitter {
  ...
  private buildDefaultArchiveSyncState(): ArchiveSyncState {
    return {
      sourceArchivePath: this.originalFilePath.toLowerCase().endsWith('.cagent') ? this.originalFilePath : null,
      isUpdated: false,
      lastExportedAt: null,
    };
  }
  ...
  private normalizeArchiveSyncState(raw: any): ArchiveSyncState {
    ...
    return {
      sourceArchivePath: typeof raw?.sourceArchivePath === 'string' ? raw.sourceArchivePath : defaults.sourceArchivePath,
      isUpdated: raw?.isUpdated === true,
      lastExportedAt: typeof raw?.lastExportedAt === 'number' ? raw.lastExportedAt : defaults.lastExportedAt,
    };
  }
  ...
  public getCloseState(): ArchiveSyncState & { liveWorkspacePath: string; isArchiveBacked: boolean } {
    return {
  ...
  private ensureDefaults(): void {
    ...
    if (!this.data.persistence) this.data.persistence = { checkpoints: {}, blobs: {}, writes: {}, restoreHeads: {} };
    ...
    try {
      ...
      if (fs.existsSync(manifestPath)) {
          ...
          this.data = {
              ...
              persistence: state.persistence || { checkpoints: {}, blobs: {}, writes: {}, restoreHeads: {} },
      ...
      console.log(`[CagentStorage] ${isManualReload ? 'Reloaded' : 'Loaded'} file with ${Object.keys(this.data.projects).length} projects and ${Object.keys(this.data.instances).length} instances.`);
  ...
  private buildRevisionSnapshotData(): CagentFileStructure {
    ...
    snapshot.instances = Object.fromEntries(
      Object.entries(snapshot.instances).map(([id, instance]) => [
        id,
        { ...instance, content: {} },
      ]),
    );
  ...
  public listCheckpointBundles(filters?: { sessionId?: string; threadId?: string }): CheckpointBundle[] {
  ...
  public async save(): Promise<void> {
    ...
    this.savePromise = this.savePromise.then(async () => {
        try {
            ...
            const stats = await fs.promises.stat(this.originalFilePath).catch(() => ({ mtimeMs: Date.now() }));
            this.lastWriteTime = stats.mtimeMs;
  ...
  public createInstance(type: string, data: Partial<CagentInstance>): CagentInstance {
    ...
    const nameExists = Object.values(this.data.instances).some(inst => 
        inst.projectId === data.projectId && inst.name === data.name
    );
  ...
  public updateInstance(id: string, partialData: Partial<CagentInstance>): CagentInstance | null {
    ...
    if (partialData.metadata !== undefined) instance.metadata = { ...instance.metadata, ...partialData.metadata };
  ...
  public deleteInstance(id: string): { deleted: boolean } {
      if (this.data.instances[id]) {
          ...
          this.instanceStore.deleteContent(id).catch(err => console.error("Failed to delete instance content files", err));
          this.triggerSave();
          return { deleted: true };
      }
  ...
  public deleteProject(id: string): { deleted: true } | null {
      ...
      public getChatSessions(): Array<{ id: string; title: string; updatedAt: number; messageCount: number }> {
