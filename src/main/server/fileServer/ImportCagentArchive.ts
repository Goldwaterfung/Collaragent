...
export class ImportCagentArchive implements StorageMigration {
  ...
  toVersion = 3
  description =
    'Extracts legacy .cagent archive and migrates monolithic in-memory structure into a sharded V3 folder format.'
    ...
    try {
      ...
      if (!fs.existsSync(cagentJsonPath)) {
        return {
          ...
          artifactsMigrated,
          warnings,
          errors: ['Missing cagent.json in target directory. Not a valid V2 archive.']
        }
      }
      ...
      if (decoded.header?.magic !== 'CAGENT') {
        return {
          ...
          artifactsMigrated,
          warnings,
          errors: ['Invalid file format: Missing magic bytes']
        }
      }
