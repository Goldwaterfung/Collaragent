...
const NODE_PREFIXED_UUID_PATTERN = /^node-([0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12})$/i;
...
export function canonicalizeGraphCanvasDTO(value: unknown): GraphCanvasDTO {
  ...
  for (const [rawKey, rawNode] of Object.entries(parsed.graph.nodes)) {
    ...
    let canonicalNodeId =
      canonicalNodeIdBySource.get(sourceKey) ??
      canonicalNodeIdBySource.get(sourceNodeId);
