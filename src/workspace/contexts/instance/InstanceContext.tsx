...
export function InstanceProvider({ children }: { children: ReactNode }) {
  ...
  const setOpenInstanceIds = useCallback((ids: string[]) => {
    ...
    setOpenInstanceIdsState((prev) =>
      areInstanceIdListsEqual(prev, normalizedIds) ? prev : normalizedIds
    )
  }, [])

  const getNextActiveInstanceId = useCallback((deletedSet: ReadonlySet<string>) => {
    const currentInstanceId = instanceIdRef.current
    if (!deletedSet.has(currentInstanceId)) {
      return currentInstanceId
    }

    const remaining = instanceSummariesRef.current.filter(
      (summary) => !deletedSet.has(summary.instanceId)
    )
    if (remaining.length === 0) {
      return DEFAULT_INSTANCE
    }

    const firstDeleted = instanceSummariesRef.current.find((summary) =>
      deletedSet.has(summary.instanceId)
    )
  ...
  const importMarkdownAsDocument = useCallback(
    async (name: string, markdownContent: string, projectId: string) => {
      try {
        // 1. Create document with unique name
        const { instanceId: newId, name: uniqueName } =
          await instanceService.createDocumentWithUniqueName(name, projectId, { isHidden: false })
  ...
  const applyRemoteIds = useCallback(
    ...
    const sub = instanceService.subscribeToOpen(async (detail) => {
      try {
        ...
        if (projectName) {
          ...
          if (!project) {
            console.warn(
              `Requested project not found: ${projectName} — aborting instance lookup for ${instanceName}`
            )
            return
          }

          const projectId = project.id
          let found = instanceSummaries.find(
            (s) => s.name === instanceName && s.projectId === projectId
          )
        ...
        if (found) {
          ...
          console.warn(
            `Requested instance not found locally: ${instanceName} (no global lookup performed)`
          )
        }
      } catch (err) {
