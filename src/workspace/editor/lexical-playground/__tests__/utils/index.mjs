...
export const IS_COLLAB_V1 = process.env.E2E_EDITOR_MODE === 'rich-text-with-collab'
export const IS_COLLAB_V2 = process.env.E2E_EDITOR_MODE === 'rich-text-with-collab-v2'
...
export const IS_TABLE_HORIZONTAL_SCROLL = process.env.E2E_TABLE_MODE !== 'legacy'
export const SAMPLE_SVG_URL = '/logo.svg'
export const SAMPLE_IMAGE_URL =
  E2E_PORT === 3000 ? '/src/images/yellow-flower.jpg' : findAsset('yellow-flower*.jpg')
...
export const YOUTUBE_SAMPLE_URL = 'https://www.youtube-nocookie.com/embed/jNQXAC9IVRw'

export function wrapTableHtml(expected, { ignoreClasses = false, ignoreDir = false } = {}) {
  return html`
    ${expected
      .replace(/<table(\s[^>]*)?>/g, (match, rawAttrs = '') => {
        const attrs = [...rawAttrs.matchAll(/(\w+)=["']([^"']*)["']/g)].map((m) => [m[1], m[2]])
        const dirAttr = attrs.find(([k]) => k === 'dir')
        const divAttrs = [
          dirAttr,
          !ignoreClasses && ['class', 'PlaygroundEditorTheme__tableScrollableWrapper']
        ]
          .filter(Boolean)
          .map(([k, v]) => `${k}="${v}"`)
        const tableAttrs = attrs.filter(([k]) => k !== 'dir').map(([k, v]) => `${k}="${v}"`)
...
export async function initialize({
  ...
  appSettings.tableHorizontalScroll = tableHorizontalScroll ?? IS_TABLE_HORIZONTAL_SCROLL
  if (isCollab) {
  ...
  appSettings.shouldAllowHighlightingWithBrackets = !!shouldAllowHighlightingWithBrackets
  ...
  if (IS_COLLAB) {
    await Promise.all(
      ['left', 'right'].map(async (name) => {
        ...
        await expect(frameLocator.locator('[data-lexical-editor="true"] p')).toBeVisible()
      })
    )
...
function removeSafariLinebreakImgHack(actualHtml) {
  return E2E_BROWSER === 'webkit'
    ? actualHtml.replaceAll(/<img (?:[^>]+ )?data-lexical-linebreak="true"(?: [^>]+)?>/g, '')
    : actualHtml
}
  ...
  return await expect(async () => {
    const actualHtml = removeDropTargetAttributes(
      removeSafariLinebreakImgHack(
        await pageOrFrame.locator('div[contenteditable="true"]').first().innerHTML()
      )
    )
    ...
    expect(actual, `innerHTML of contenteditable in ${frameName} did not match`).toEqual(expected)
...
export async function assertTableHTML(
  ...
  return await assertHTML(
    page,
    IS_TABLE_HORIZONTAL_SCROLL ? wrapTableHtml(expectedHtml, options) : expectedHtml,
...
async function pasteWithClipboardDataFromPageOrFrame(pageOrFrame, clipboardData, editorSelector) {
...
export async function pasteFromClipboard(
  ...
  await pasteWithClipboardDataFromPageOrFrame(getPageOrFrame(page), clipboardData, editorSelector)
}
...
export async function dragMouse(page, fromBoundingBox, toBoundingBox, opts = {}) {
  const {
...
export async function prettifyHTML(string, { ignoreClasses, ignoreInlineStyles, ignoreDir } = {}) {
  let output = string
...
export async function clickTableCellActiveButton(page) {
  await click(page, '.table-cell-action-button-container--active > .table-cell-action-button')
}
