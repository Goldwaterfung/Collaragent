...
test.describe('Tab', () => {
  ...
  test(`can tab + IME`, { tag: '@flaky' }, async ({ page, isPlainText, browserName }) => {
    // CDP session is only available in Chromium
    test.skip(isPlainText || browserName === 'firefox' || browserName === 'webkit')
  ...
  test('can go to start of line after a tab character', async ({ page, isPlainText }) => {
    test.skip(isPlainText)
    await focusEditor(page)
