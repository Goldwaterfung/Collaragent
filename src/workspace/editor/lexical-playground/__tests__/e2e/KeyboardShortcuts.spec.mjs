...
const getSelectedFormat = async (page) => {
  return await textContent(page, '.toolbar-item.block-controls > .text.dropdown-button-text')
}
...
const isDropdownItemActive = async (page, dropdownItemIndex) => {
  return await evaluate(
    ...
    async (_dropdownItemIndex) => {
      await document
        .querySelector('button[aria-label="Formatting options for additional text styles"]')
        .click()
...
test.describe('Keyboard shortcuts', () => {
  ...
  formatTestCases.forEach(({ format, applyShortcut, canToggle }) => {
    test(`Can use ${format} format with the shortcut`, async ({ page, isPlainText }) => {
  ...
  alignmentTestCases.forEach(({ alignment, applyShortcut }, index) => {
    test(`Can use ${alignment} with the shortcut`, async ({ page, isPlainText }) => {
      await focusEditor(page)
      await applyShortcut(page)
  ...
  additionalStylesTestCases.forEach(({ applyShortcut, style }, dropdownItemIndex) => {
    test(`Can use ${style} with the shortcut`, async ({ page, isPlainText }) => {
      await focusEditor(page)
      await applyShortcut(page)
  ...
  test('Can increase and decrease font size with the shortcuts', async ({ page, isPlainText }) => {
    await focusEditor(page)
  ...
  test('Can clear formatting with the shortcut', async ({ page, isPlainText }) => {
    await focusEditor(page)
  ...
  test('Can toggle Insert Code Block with the shortcut', async ({ page, isPlainText }) => {
    await focusEditor(page)
  ...
  test('Can indent and outdent with the shortcuts', async ({ page, isPlainText }) => {
    await focusEditor(page)
    await page.keyboard.type('abc')
    await indent(page, 3)
