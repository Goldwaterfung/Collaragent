...
test.describe.parallel('TextFormatting', () => {
  ...
  test(`Can select text and underline+strikethrough`, async ({ page, isPlainText }) => {
    ...
    await assertHTML(
      ...
      html`
        <p class="PlaygroundEditorTheme__paragraph" dir="auto">
          <span data-lexical-text="true">Hello</span>
          <span class="PlaygroundEditorTheme__textUnderlineStrikethrough" data-lexical-text="true">
            world
          </span>
          <span data-lexical-text="true">!</span>
        </p>
  ...
  capitalizationFormats.forEach(({ className, format, applyCapitalization }) => {
    test(`Can select text and change it to ${format}`, async ({ page, isPlainText }) => {
      test.skip(isPlainText)
  ...
  capitalizationFormats.forEach(({ format, className, applyCapitalization }) => {
    capitalizationResettingTestCases.forEach(({ key, expectedFinalHTML }) => {
      test(`Pressing ${key} resets ${format} format`, async ({ page, isPlainText }) => {
        test.skip(isPlainText)
        ...
        await assertHTML(page, expectedFinalHTML.replace('$formatClassName', className))
      })
    })
  })

  test(`Can select text and increase the font-size`, async ({ page, isPlainText }) => {
    test.skip(isPlainText)
  ...
  test(`Can select text and decrease the font-size`, async ({ page, isPlainText }) => {
    test.skip(isPlainText)
  ...
  test(`Can select text and change the font-size and font-family`, async ({
    ...
    await assertHTML(
      ...
      html`
        <p class="PlaygroundEditorTheme__paragraph" dir="auto">
          <span data-lexical-text="true">Hello</span>
          <span style="font-size: 17px; font-family: Georgia;" data-lexical-text="true">
            world
          </span>
          <span data-lexical-text="true">!</span>
        </p>
    ...
    await assertHTML(
      ...
      html`
        <p class="PlaygroundEditorTheme__paragraph" dir="auto">
          <span data-lexical-text="true">Hello</span>
          <span style="font-size: 15px; font-family: Georgia;" data-lexical-text="true">
            world
          </span>
          <span data-lexical-text="true">!</span>
        </p>
  ...
  test(`Can select text with different size and update font size by entering the value`, async ({
    ...
    await assertHTML(
      ...
      html`
        <p class="PlaygroundEditorTheme__paragraph" dir="auto">
          <span style="font-size: 20px;" data-lexical-text="true"> Hello world! </span>
        </p>
      `
    )
  })
  ...
  test('Multiline selection format ignores new lines', async ({ page, isPlainText, isCollab }) => {
    test.skip(isPlainText)
    let leftFrame = page
    if (isCollab) {
    ...
    await expect(leftFrame.locator('.toolbar-item[title^="Underline"]')).toHaveClass(/active/)
  })
})
