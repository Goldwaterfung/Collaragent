...
import { moveLeft, selectCharacters, toggleBold, undo } from '../keyboardShortcuts/index.mjs'
import {
  assertHTML,
  assertSelection,
  click,
...
test.describe('Collaboration', () => {
  ...
  test('Undo with collaboration on', async ({ isRichText, page, isCollab, browserName }) => {
    test.skip(!isCollab)
  ...
  test('Remove dangling text from YJS when there is no preceding text node', async ({
    ...
    await sleep(1050)
    await page.frameLocator('iframe[name="right"]').locator('[data-lexical-editor="true"]').focus()
    ...
    await page.evaluate(() => {
      document.querySelector('iframe[name="left"]').contentDocument.location.reload()
    })
  ...
  test('Undo/redo where text node is split by inline element node', async ({
    ...
    await page.frameLocator('iframe[name="left"]').getByLabel('Insert link').first().click()
