...
import { evaluate, expect, focusEditor, initialize, locate, test } from '../utils/index.mjs'
...
test.describe('SelectionAlwaysOnDisplay', () => {
  ...
  test(`retain selection works`, async ({ page, isPlainText, browserName }) => {
    ...
    const { distance, widthDifference, heightDifference } = await evaluate(page, () => {
      ...
      const editorSpan = document.querySelector('[contenteditable="true"] span')
      const fakeSelection = document.querySelector('[style*="background: highlight"]')
