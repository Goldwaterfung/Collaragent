...
test.describe('Regression tests for #7319', () => {
  ...
  test(`deleteCharacter after hr with RangeSelection`, async ({ page, isCollab, isPlainText }) => {
    ...
    async function assertHR(count) {
      const expectedHtml = html`
        ...
        ${count > 0 ? '<div contenteditable="false" data-lexical-cursor="true"></div>' : ''}
      `
  ...
  test(`deleteCharacter after hr with NodeSelection`, async ({ page, isCollab, isPlainText }) => {
    test.skip(isCollab || isPlainText)
    await focusEditor(page)
    await page.keyboard.press('Enter')
    const hrCount = 3
