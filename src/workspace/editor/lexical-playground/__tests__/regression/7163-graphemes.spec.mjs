...
test.describe('Grapheme deleteCharacter', () => {
  ...
  ;[
    ...
    },
    {
      backspaceCount: 1,
      caretDistance: 1,
      description: 'Multiple codepoint Emoji with variation selector entirely in the BMP',
      grapheme: '\u2764\ufe0f'
    },
    {
      backspaceCount: 1,
  ...
  ].forEach(({ backspaceCount, caretDistance, description, grapheme, skip = false }) => {
    test(description, async ({ page, browserName, isCollab, isPlainText }) => {
      ...
      await assertSelection(page, selectionFromOffset(initialOffset, initialPath))
      await assertHTML(page, expectedInitialHTML, undefined, {
        ignoreClasses: true,
        ignoreInlineStyles: true
      })
