    ...
    (testEnv) => {
      describe('exportDOM', () => {
        it('exports with inline grid-template-columns style', async () => {
          ...
          const htmlOutput = editor.read(() => $generateHtmlFromNodes(editor, null))
          expectHtmlToBeEqual(
            htmlOutput,
            html`
              <div
      ...
      describe('importDOM', () => {
        it('imports layout container from inline style', async () => {
          ...
          editor.read(() => {
            ...
            expect((container as LayoutContainerNode).getTemplateColumns()).toBe('1fr 1fr')
          })
        })
        ...
        it('returns null for div without data-lexical-layout-container', async () => {
          const { editor } = testEnv
          const parser = new DOMParser()
          const htmlString = '<div style="grid-template-columns: 1fr 1fr;"></div>'
          const dom = parser.parseFromString(htmlString, 'text/html')
          await editor.update(() => {
            const nodes = $generateNodesFromDOM(editor, dom)
            $getRoot().select()
      ...
      describe('export/import round-trip', () => {
        it('preserves templateColumns through HTML round-trip', async () => {
          ...
          const exportedHtml = editor.read(() => $generateHtmlFromNodes(editor, null))
          ...
          editor.read(() => {
            ...
            const container = root.getFirstChild()
            expect(container).toBeInstanceOf(LayoutContainerNode)
            expect((container as LayoutContainerNode).getTemplateColumns()).toBe(templateColumns)
          })
        })
