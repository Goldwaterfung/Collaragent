...
function createTableNodeFromBlock(block: Block, comments: Record<string, Comment>): ElementNode {
  ...
  for (const row of rows) {
    ...
    for (const cell of row.cells) {
      ...
      const cellNode = $createTableCellNode(headerState, cell.colSpan)
...
function createBlockNode(block: Block, comments: Record<string, Comment>): ElementNode {
  ...
  // Populate children
  const runs: InlineRun[] = children ?? (typeof content === 'string' ? [{ text: content }] : [])
...
function createListItemFromBlock(block: Block, comments: Record<string, Comment>): ListItemNode {
...
function createNodesFromRun(run: InlineRun): LexicalNode[] {
  ...
  parts.forEach((part, index) => {
    ...
    if (part || parts.length === 1) {
      // Keep empty string if it's the only part
