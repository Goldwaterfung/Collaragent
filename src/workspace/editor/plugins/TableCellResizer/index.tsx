...
const ACTIVE_RESIZER_COLOR = 'var(--color-primary)';

function TableCellResizer({editor}: {editor: LexicalEditor}): JSX.Element {
  const resizerRef = useRef<HTMLDivElement | null>(null);
  const tableRectRef = useRef<ClientRect | null>(null);
  const draggingElemRef = useRef<HTMLDivElement | null>(null);
  ...
  const resetState = useCallback(() => {
    ...
    if (
      draggingElemRef.current != null &&
      pointerIdRef.current != null &&
      draggingElemRef.current.hasPointerCapture(pointerIdRef.current)
    ) {
      draggingElemRef.current.releasePointerCapture(pointerIdRef.current);
    }
    draggingElemRef.current = null;
    pointerIdRef.current = null;
  }, []);
  ...
  useEffect(() => {
    const onPointerMove = (event: PointerEvent) => {
      ...
      const cell = cellDOM ? getDOMCellFromTarget(cellDOM) : null;

      if (cell && activeCell?.elem !== cell.elem) {
  ...
  const toggleResize = useCallback(
    (
      ...
      (event) => {
        ...
        draggingElemRef.current = event.currentTarget;
    ...
    if (activeCell) {
      ...
      const styles: Record<string, CSSProperties> = {
        bottom: {
          ...
          top: `${window.scrollY + top + height - zoneWidth}px`,
          width: `${width}px`,
          zIndex: 11,
        },
        right: {
          ...
          cursor: 'col-resize',
          height: `${height}px`,
          left: `${window.scrollX + left + width - zoneWidth}px`,
      ...
      if (draggingDirection && pointerCurrentPos && tableRect) {
        if (isHeightChanging(draggingDirection)) {
          ...
          styles[draggingDirection].top = `${
            window.scrollY + pointerCurrentPos.y
          }px`;
          ...
          styles[draggingDirection].left = `${
            window.scrollX + pointerCurrentPos.x
          }px`;
          styles[draggingDirection].width = '3px';
          styles[draggingDirection].height = `${tableRect.height}px`;
        }

        styles[draggingDirection].backgroundColor = 'var(--color-primary)';
        styles[draggingDirection].mixBlendMode = 'unset';
      } else if (!draggingDirection && hoveredDirection === 'right') {
        const highlightWidth = 2;
        const highlightStart = zoneWidth - highlightWidth;
