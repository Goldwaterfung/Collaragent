...
function setTargetLine(
  ...
  if (mouseY - targetBlockElemTop > targetBlockElemHeight / 2) {
    lineTop += targetBlockElemHeight / scaleY + parseFloat(targetStyle.marginBottom)
  } else {
  ...
  targetLineElem.style.width = `${
    anchorWidthLocal - ((TEXT_BOX_HORIZONTAL_PADDING - SPACE) / scaleX) * 2
  }px`
...
function useDraggableBlockMenu(
  ...
  useEffect(() => {
    const container = scrollerElem ?? anchorElem

    function onDragLeave(event: DragEvent) {
      const related = event.relatedTarget
      if (!isHTMLElement(related) || !container.contains(related)) {
