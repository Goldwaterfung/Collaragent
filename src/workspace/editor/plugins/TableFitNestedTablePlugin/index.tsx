...
function calculateHorizontalInsets(dom: HTMLElement, editorWindow: Window): number {
  ...
  const borderRightWidth = computedStyle.getPropertyValue('border-right-width') || '0px'
...
function $calculateResizeRootTables(tables: ReadonlySet<TableNode>): ReadonlyArray<TableNode> {
  const inputTables: ReadonlySet<LexicalNode> = tables
  const roots: TableNode[] = []
  for (const table of tables) {
    if ($findMatchingParent(table, (n) => n !== table && inputTables.has(n)) === null) {
      roots.push(table)
    }
  }
  return roots
...
function $resizeDOMColWidthsToFit(editor: LexicalEditor, node: TableNode): void {
  ...
  for (const table of allNestedTables) {
    ...
    const parentShadowRoot = $findMatchingParent(tableParent, $isRootOrShadowRoot)
    ...
    const horizontalInsets = calculateHorizontalInsets(fitContainer, editorWindow)
