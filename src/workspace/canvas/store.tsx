...
const maybeMergeSnapshot = (state: CanvasState, command: CanvasCommand): { shouldPush: boolean; nextHistory: CanvasState['history'] } => {
  const now = Date.now();

  if (command.type !== 'MoveNode' && command.type !== 'ResizeNode') {
    return {
      shouldPush: true,
      nextHistory: {
        ...state.history,
  ...
  const sameStream =
    withinWindow && last?.commandType === command.type && last?.nodeId === nodeId;
...
function canvasReducer(state: CanvasState, action: CanvasAction): CanvasState {
  switch (action.type) {
    ...
    case 'COMMANDS': {
      ...
      const mergeInfo = commands.length === 1 ? maybeMergeSnapshot(state, commands[0]) : { shouldPush: true, nextHistory: { ...state.history, lastMerge: undefined } };
...
export const CanvasProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await instanceService.getById(canvasInstanceId) as unknown as InstanceWithPayload;
        ...
        const { graph, layoutByNodeId } = deserializeCanvas(content, { graphId: `graph-${canvasInstanceId}` });
        dispatch({
          type: 'HYDRATE_CANVAS',
          payload: {
            graph,
  ...
  const dispatchCommand = useCallback((command: CanvasCommand) => {
  ...
  const applyWorkspaceRestore = (snapshotDto: GraphCanvasDTO, commands: SharedCanvasCommand[]) => {
    const { graph, layoutByNodeId } = deserializeCanvas(snapshotDto, { graphId: `graph-${canvasInstanceId}` });
  ...
  const dispatchTransaction = useCallback((commands: CanvasCommand[]) => {
  ...
  return (
    <CanvasContext.Provider value={{ state, dispatch, dispatchCommand, dispatchTransaction, applyWorkspaceRestore, subscribe }}>
      {children}
