...
export function normalizeSignedOptions(
  options: Partial<SignedOptions> | undefined
): Required<SignedOptions> {
  const signedMode: SignedMode = options?.signedMode ?? 'penalty'
  const lambdaRaw = options?.lambda
  const lambda =
    Number.isFinite(lambdaRaw) && (lambdaRaw as number) >= 0 ? (lambdaRaw as number) : 1
