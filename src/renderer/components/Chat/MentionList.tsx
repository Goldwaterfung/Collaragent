  ...
  return createPortal(
    ...
    >
      {suggestions.map((item, index) => (
        ...
        >
          <span
            className={`flex-shrink-0 ${index === selectedIndex ? 'text-black' : 'text-[var(--ev-c-text-2)]'}`}
          >
            {item.type === 'canvas' ? (
          ...
          <div className="flex flex-col min-w-0 overflow-hidden">
            ...
            <span
              className={`text-[10px] truncate ${index === selectedIndex ? 'text-black/70' : 'text-[var(--ev-c-text-3)]'}`}
            >
