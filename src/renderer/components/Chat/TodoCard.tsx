...
export const TodoCard: React.FC<TodoCardProps> = ({ todos, className = '' }) => {
  ...
  return (
    <div
      className={`bg-surface-50/95 border-t border-surface-200 backdrop-blur-sm p-3 ${className}`}
    >
      <button
    ...
    <div className="flex items-start gap-2 text-[11px]">
      <div
        className={`
            ...
            `}
      >
        {isCompleted && <CheckIcon className="w-2 h-2 text-white" />}
        {isInProgress && <div className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />}
      </div>
      <span
        className={`leading-4 transition-colors ${isCompleted ? 'text-gray-400 line-through' : 'text-gray-700'}`}
      >
        {todo.content}
      </span>
    </div>
