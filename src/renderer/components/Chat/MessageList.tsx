...
const MessageListComponent: React.FC<MessageListProps> = ({
    ...
    const sortedBundles = [...checkpointBundles].sort((a, b) => b.createdAt.localeCompare(a.createdAt));
    ...
    messages.forEach((msg, index) => {
        ...
        const groupedBlocks = msg.role === 'assistant'
            ? groupBlocksByTodos(msg.blocks, msg.toolCalls)
            : [];
        ...
        items.push(
            ...
            >
                <div className="space-y-4">
                    ...
                    {msg.role === 'assistant' && groupedBlocks.map((group, gIdx) => (
                        <ProgressContainer key={gIdx} inProgressTodos={group.inProgressTodos}>
                            <div className="space-y-4">
                                {group.blocks.map((block, i) => (
                                    block.type === 'text' ? <div key={i}>{renderContent(block.content)}</div> :
                                    block.type === 'reasoning' ? <ReasoningCard key={i} content={block.content} /> :
                                        (() => {
