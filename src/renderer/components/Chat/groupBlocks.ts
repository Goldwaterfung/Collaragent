 ...
 */
export function groupBlocksByTodos(
  blocks: MessageBlock[] | undefined,
  toolCalls: ToolCall[] | undefined
): BlockGroup[] {
  const groups: BlockGroup[] = []
  let currentTodos: TodoItemData[] = []
  ...
  blocks.forEach((block) => {
    if (block.type === 'tool') {
      ...
      if (tool?.name === 'write_todos') {
        ...
        currentTodos = Array.isArray(args?.todos) ? (args.todos as TodoItemData[]) : []
