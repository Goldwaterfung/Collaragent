    ...
    <>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-full hover:bg-surface-200 text-(--ev-c-text-3) hover:text-(--ev-c-text-1) transition-colors"
          >
            <svg
              width="14"
              height="14"
        ...
        <div className="space-y-4">
          ...
          <StatRow
            ...
            value={
              activeUsage
                ? activeUsage.cachedInputTokens || 0
                : latestTurnUsage.cachedInputTokens || 0
            }
          ...
          {((latestTurnUsage.reasoningTokens || 0) > 0 ||
            (activeUsage?.reasoningTokens || 0) > 0) && (
            <StatRow
              label="Reasoning"
              value={
                activeUsage
                  ? activeUsage.reasoningTokens || 0
                  : latestTurnUsage.reasoningTokens || 0
              }
          ...
          <div className="pt-4 mt-2 border-t border-surface-200">
            <div className="flex justify-between items-center">
              <span className="text-xs font-medium text-(--ev-c-text-3) uppercase tracking-wider">
                Total context
              </span>
        ...
        <p className="mt-6 text-[10px] text-(--ev-c-text-3) text-center leading-relaxed">
          Statistics represent the token usage for the most recent model turn, indicating current
...
const StatRow: React.FC<{ label: string; value: number; active?: number; color?: string }> = ({
  label,
  value,
  active,
  color
  ...
  <div className="flex justify-between items-baseline group">
    ...
    <div className="flex items-baseline gap-2">
      {active !== undefined && (
        <span
          className={`text-[10px] font-mono font-medium animate-pulse ${color || 'text-(--color-primary)'}`}
        >
          +{active}
        </span>
      )}
