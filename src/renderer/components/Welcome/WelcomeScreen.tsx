...
export function WelcomeScreen() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[var(--color-surface-50)] text-[var(--ev-c-text-1)] select-none overflow-y-auto custom-scrollbar">
      <div className="min-h-full flex flex-col items-center justify-center w-full py-8 sm:py-12 px-4 sm:px-6">
        {/* Header / Brand */}
        <div className="flex-none text-center mb-8 animate-in fade-in slide-in-from-top-4 duration-500">
        ...
        {/* Main Content Area */}
        <div className="flex flex-col items-center w-full max-w-2xl gap-8 animate-in fade-in zoom-in-95 duration-500 delay-150">
          {/* Action Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 w-full">
            <button
              onClick={handleCreateFile}
              className="group relative flex flex-col items-center justify-center p-6 sm:p-8 rounded-2xl border border-[var(--color-surface-200)] bg-[var(--color-surface-100)] hover:bg-[var(--color-surface-50)] hover:border-[var(--color-primary)] hover:shadow-lg hover:shadow-[var(--color-primary)]/10 transition-all duration-300 cursor-pointer text-left overflow-hidden focus:outline-none"
            >
              ...
              <p className="text-[var(--ev-c-text-3)] text-center text-sm">
                Start a fresh workspace for your agentic workflows.
              </p>
            </button>

            <button
              onClick={handleOpenFile}
              className="group relative flex flex-col items-center justify-center p-6 sm:p-8 rounded-2xl border border-[var(--color-surface-200)] bg-[var(--color-surface-100)] hover:bg-[var(--color-surface-50)] hover:border-[var(--color-accent)] hover:shadow-lg hover:shadow-[var(--color-accent)]/10 transition-all duration-300 cursor-pointer text-left overflow-hidden focus:outline-none"
            >
              ...
              <p className="text-[var(--ev-c-text-3)] text-center text-sm">
                Continue working on your stored{' '}
                <code className="bg-[var(--color-surface-200)] px-1 rounded text-xs select-text">
                  .cagent
                </code>{' '}
                workspaces.
              </p>
          ...
          <div className="w-full animate-in fade-in slide-in-from-bottom-8 duration-700 delay-300">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-[var(--ev-c-text-2)] uppercase tracking-wider">
                Recent Workspaces
              </h2>
              {recentFiles.length > 0 && (
                <span className="text-xs text-[var(--ev-c-text-3)]">
                  {recentFiles.length} found
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2 opacity-90">
              {recentFiles.length === 0 ? (
                <div className="w-full p-4 rounded-lg border border-dashed border-[var(--color-surface-300)] bg-[var(--color-surface-50)] text-center">
                  <p className="text-[var(--ev-c-text-3)] text-sm opacity-60">
                ...
                recentFiles.map((file) => (
                  <div
                    ...
                    className="group flex items-center justify-between p-3 rounded-lg border border-[var(--color-surface-200)] bg-[var(--color-surface-100)] hover:border-[var(--color-primary)] cursor-pointer transition-colors focus:outline-none"
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex flex-col">
                        <span className="font-medium text-sm text-[var(--ev-c-text-1)]">
                          {file.name}
                        </span>
                        <span
                          className="text-xs text-[var(--ev-c-text-3)] truncate max-w-[300px]"
                          title={file.path}
                        >
                          {file.path}
                        </span>
        ...
        {/* Footer */}
        <div className="flex-none pt-8 pb-2 text-center">
