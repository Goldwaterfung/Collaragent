    ...
    >
      ...
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <span
            className={`text-[10px] text-(--ev-c-text-3) transition-transform duration-200 ${isExpanded ? 'rotate-90' : ''}`}
          >
            ▶
          </span>
          ...
          {isEmpty && (
            <span className="text-[10px] text-(--ev-c-text-3) italic font-normal">Empty</span>
          )}
          {isDragOver && (
            <span className="text-[9px] font-bold text-primary ml-2 animate-pulse">DROP HERE</span>
          )}
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation()
              onOpenCreateModal('document', pid)
            }}
            className="p-1 hover:bg-surface-200 rounded-md text-(--ev-c-text-2) flex items-center justify-center transform hover:scale-110 transition-transform"
            title="Add Document"
          >
        ...
        <div className="pl-3 pr-2 pb-2 mt-1 relative">
          ...
          <div className="space-y-3 pl-3">
            {group.canvases.length > 0 && (
              <div>
                <div className="text-[9px] font-bold text-(--ev-c-text-3) uppercase mb-1 px-2 tracking-wider">
                  Canvases
                </div>
            ...
            <div
              onDragOver={(e) => {
                e.preventDefault()
                setIsPromoteDragOver(true)
              }}
              onDragLeave={() => setIsPromoteDragOver(false)}
              onDrop={(e) => {
                setIsPromoteDragOver(false)
                handleDrop(e)
              }}
            ...
            >
              <div className="text-[9px] font-bold text-(--ev-c-text-3) uppercase mb-1 px-2 tracking-wider flex justify-between items-center">
                <span>Documents</span>
                {isPromoteDragOver && (
                  <span className="text-[8px] animate-pulse">MOVE TO LIBRARY</span>
                )}
              </div>
              {group.documents.length > 0
                ? group.documents.map((s) => renderInstanceItem(s, 'document'))
                : !isPromoteDragOver && (
                    <div className="text-[9px] text-(--ev-c-text-3) px-2 italic">
                      Drag nodes here to promote
                    </div>
                  )}
            </div>
          </div>
...
export function InstanceManager({ onSelect }: { onSelect?: () => void }) {
  ...
  const groupedInstances = useMemo(() => {
    const groups: Record<
      string,
      { canvases: any[]; documents: any[]; nodesByCanvasId: Record<string, any[]> }
    > = {}
  ...
  const renderInstanceItem = (summary: any, type: 'canvas' | 'document', depth = 0) => {
    ...
    const hasChildren =
      type === 'canvas' && groupedInstances[summary.projectId]?.nodesByCanvasId[id]?.length > 0
      ...
      <div key={id} className="mb-0.5">
        ...
        >
          <div className="flex items-center gap-2 truncate flex-1 leading-none">
            ...
            <span
              className={`opacity-70 ${id === instanceId || contextMenu?.id === id ? 'text-(--ev-c-black)' : ''}`}
            >
              {type === 'canvas' ? (
                <CanvasIcon width={14} height={14} />
              ) : (
        ...
        {type === 'canvas' && isExpanded && (
          <div className="mt-0.5">
            {groupedInstances[summary.projectId]?.nodesByCanvasId[id]?.map((node) =>
              renderInstanceItem(node, 'document', depth + 1)
            )}
          </div>
        )}
      </div>
  ...
  return (
    <Fragment>
      <div className="flex flex-col h-full bg-surface-50 border-r border-surface-200 w-full overflow-hidden relative">
        <div className="px-3 py-3 flex justify-between items-center mb-1">
          <h2 className="font-bold text-xs text-(--ev-c-text-1) uppercase tracking-widest pl-1">
            Workspace
          </h2>
