    ...
    <div className="tool-list p-4 sm:p-6 lg:p-8 border border-surface-200 rounded-xl sm:rounded-2xl bg-white shadow-sm">
      <div className="mb-4 sm:mb-5 lg:mb-6">
        ...
        <p className="text-xs sm:text-sm text-black/50 mt-1">
          Select tools available directly to the main agent
        </p>
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex-1 min-w-0 mr-3">
                ...
                <div className="text-xs sm:text-sm text-black/50 font-mono mt-0.5 truncate">
                  {tool.langchainTool}
                </div>
              </div>
            ...
            {tool.enabled && tool.requireAPI && <ToolApiKeyInput toolId={tool.id} />}
          </div>
        ))}
        {tools.length === 0 && (
          <div className="md:col-span-2">
            <p className="text-black/50 text-sm sm:text-base text-center py-6 sm:py-8">
              No tools available.
            </p>
          </div>
        )}
  ...
  return (
    <div className="mt-2 pt-2 border-t border-surface-200">
      <div className="flex items-center justify-between mb-1">
        <label className="text-xs font-medium text-black/60 block">API Key Required</label>
        {isSaved && (
