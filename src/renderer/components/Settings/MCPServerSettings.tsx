...
export const MCPServerSettings: React.FC<MCPServerSettingsProps> = ({ mcpServers, onRefresh }) => {
  ...
  const handleToggleServerTool = async (
    ...
    if (newServer.transport.type === 'stdio') {
      return (
        <div className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-black/60 uppercase tracking-wider mb-1">
              Command
            </label>
            <input
              type="text"
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-3">
                <span
                  className={`w-2 h-2 rounded-full ${server.enabled ? 'bg-green-500' : 'bg-surface-300'}`}
                />
            ...
            {server.enabled && (
              <div className="mt-4 pt-4 border-t border-surface-200">
                  ...
                  <div className="space-y-2 mt-2 max-h-64 overflow-y-auto pr-1 stylish-scrollbar">
                    {serverTools[server.id].map((tool: { name: string; description?: string }) => {
                      ...
                      return (
                        <div
                          key={tool.name}
                          className="flex flex-row items-center justify-between p-2.5 bg-white rounded-lg border border-surface-100 hover:border-surface-200 transition-colors"
                        >
                          <div className="flex-1 min-w-0 pr-4">
                            ...
                            {tool.description && (
                              <div className="text-xs text-black/50 line-clamp-2 mt-0.5">
                                {tool.description}
                              </div>
                            )}
                          </div>
                          <label className="relative inline-flex items-center cursor-pointer shrink-0">
                            <input
                              ...
                              checked={!isDisabled}
                              onChange={(e) =>
                                handleToggleServerTool(server, tool.name, e.target.checked)
                              }
                            />
                ...
                {serverTools[server.id] && serverTools[server.id].length === 0 && (
                  <div className="text-xs text-black/40 italic p-2 bg-white rounded border border-surface-100">
          ...
          <div className="p-5 bg-white border-2 border-primary/30 rounded-xl shadow-lg space-y-4 animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex items-center gap-6 p-3 bg-surface-50 rounded-lg border border-surface-100">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="requireAPI-new" className="text-xs font-medium text-black/70">
                  Secure API Key
                </label>
              </div>
              ...
              {newServer.requireAPI && (
                <div className="flex-1 flex items-center gap-2">
                  <label className="text-[10px] font-bold text-black/40 uppercase whitespace-nowrap">
                    Env Var
                  </label>
                  <input
                    type="text"
...
const ToolApiKeyInput: React.FC<{ toolId: string; label?: string }> = ({
  toolId,
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1">
        <label className="text-xs font-medium text-black/60 block">{label}</label>
        {isSaved && (
          <span className="text-[10px] text-green-600 font-bold animate-pulse">SAVED</span>
        )}
        {!isSaved && hasKey && (
          <span className="text-[10px] text-primary font-bold opacity-60">STORED</span>
        )}
      </div>
