...
export const ModelSelector: React.FC<ModelSelectorProps> = ({ currentConfig, onUpdate }) => {
  ...
  const isCatalogModel = availableModels.some(
    (m) => m.id === currentConfig.modelId && m.provider === currentConfig.provider
  )
  const [isManual, setIsManual] = useState(!isCatalogModel && !!currentConfig.modelId)
  const [customModelId, setCustomModelId] = useState(isCatalogModel ? '' : currentConfig.modelId)
  ...
  useEffect(() => {
    ...
    if (availableModels.length > 0) {
      ...
      setIsManual(!matchesCatalog)
      if (!matchesCatalog) {
        setCustomModelId(currentConfig.modelId)
      }
    }
  ...
  const handleSave = async () => {
    ...
    setMessage(null)
    try {
      const targetModelId = isManual ? customModelId.trim() : modelId
      if (!targetModelId) {
        setMessage({ type: 'error', text: 'Please specify a valid Model ID.' })
        setLoading(false)
        return
      }

      // Find the selected model info to get its preset parameters if catalog model
  ...
  return (
    <div className="model-selector p-4 sm:p-6 lg:p-8 border border-surface-200 rounded-xl sm:rounded-2xl bg-white shadow-sm">
      <h3 className="text-base sm:text-lg lg:text-xl font-semibold mb-4 sm:mb-5 lg:mb-6 text-black">
        Model Configuration
      </h3>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-5 lg:gap-6">
        <div className="lg:col-span-1">
          ...
          <select
            value={provider}
            onChange={(e) => {
              const newProvider = e.target.value as ModelConfig['provider']
        ...
        <div className="lg:col-span-1">
          ...
          <select
            value={isManual ? '__manual__' : modelId}
            onChange={(e) => {
              if (e.target.value === '__manual__') {
                setIsManual(true)
                if (!customModelId) setCustomModelId(modelId)
              } else {
                setIsManual(false)
          ...
          >
            ...
            <optgroup label="Custom / Manual">
              <option value="__manual__">Manual Configuration (Custom Model)...</option>
            </optgroup>
          </select>
        </div>

        {isManual && (
          <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 sm:p-5 bg-surface-50 border border-surface-200 rounded-xl">
            <div>
              <label className="block text-sm font-medium mb-2 text-black">
                Custom Model ID <span className="text-black/50">(API Identifier)</span>
              </label>
              <input
                type="text"
                value={customModelId}
                onChange={(e) => setCustomModelId(e.target.value)}
                className="w-full p-3 border border-surface-200 rounded-xl bg-white text-black text-sm sm:text-base placeholder:text-black/40 focus:outline-none transition-shadow"
                placeholder="e.g. deepseek-chat, gpt-4o-2024-11-20"
                required
              />
              <p className="text-xs text-black/50 mt-1.5">
                The exact model string passed to the provider API.
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium mb-2 text-black">
              ...
              <input
                ...
                className="w-full p-3 border border-surface-200 rounded-xl bg-white text-black text-sm sm:text-base placeholder:text-black/40 focus:outline-none transition-shadow"
                placeholder="e.g. DeepSeek V3 (Custom)"
              />
        ...
        {message && (
          <div
            className={`lg:col-span-2 p-3 rounded-xl text-sm font-medium ${message.type === 'success' ? 'bg-surface-100 text-black border border-surface-200' : 'bg-surface-300 text-black border border-primary'}`}
          >
            {message.text}
          </div>
        )}
