...
**Not using Python or JS/TS?** Ingest experiment traces directly with
[OpenTelemetry experiment attributes](/docs/evaluation/experiments/experiments-via-opentelemetry).
The Experiment runner SDK sets those attributes for you.
...
```typescript
import { LangfuseClient, LangfuseMediaReference } from '@langfuse/client'
...
const result = await langfuse.experiment.run({
  ...
  data: testDataset,
  task: myTask,
  evaluators: [factualityEvaluator, levenshteinEvaluator, customFactualityEvaluator]
})
...
<Frame className="max-w-lg">
  ![Remote experiment trigger setup form with request signing and custom headers](/images/docs/trigger-remote-experiment-2.png)
</Frame>
...
This page is part of the [Langfuse](https://langfuse.com) documentation, published as plain Markdown for AI agents. Every page is available as Markdown by appending `.md` to its URL, or by sending an `Accept: text/markdown` header. This page: `https://langfuse.com/docs/evaluation/experiments/experiments-via-sdk.md`.
