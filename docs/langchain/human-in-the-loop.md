...
The Human-in-the-Loop (HITL) [middleware](docs/middleware/built-in#human-in-the-loop) lets you add human oversight to agent tool calls.
...
It does this by checking each tool call against a configurable policy. If intervention is needed, the middleware issues an [interrupt] that halts execution. The graph state is saved using LangGraph's [persistence layer](docs/langgraph/persistence), so execution can pause safely and resume later.
...
The [middleware](docs/langchain/middleware/built-in-middleware.md) defines three built-in ways a human can respond to an interrupt:
...
To use HITL, add the [middleware](docs/langchain/middleware/built-in-middleware) to the agent's `middleware` list when creating the agent.
...
See the [Streaming](docs/langchain/streaming) guide for more details on stream modes.
...
For more specialized workflows, you can build custom HITL logic directly using the [interrupt] primitive and [middleware](docs/langchain/middleware) abstraction.
