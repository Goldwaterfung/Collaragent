...
The simplest way to use messages is to create message objects and pass them to a model when [invoking](docs/langchain/models#invocation).
...
- [Tool message](#tool-message) - Represents the outputs of [tool calls](docs/langchain/models#tool-calling)
...
const response = await model.invoke([new HumanMessage('What is machine learning?')])
```
...
When models make [tool calls](docs/langchain/models#tool-calling), they're included in the [`AIMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.AIMessage.html):
...
Other structured data, such as reasoning or citations, can also appear in message [content](docs/langchain/messages#message-content).
...
* [Streaming tokens from chat models](docs/langchain/models#stream)
* [Streaming tokens and/or steps from agents](docs/langchain/streaming)
</Note>
...
For models that support [tool calling](docs/langchain/models#tool-calling), AI messages can contain tool calls. Tool messages are used to pass the results of a single tool execution back to the model.

[Tools](docs/langchain/tools) can generate [`ToolMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.ToolMessage.html) objects directly. Below, we show a simple example. Read more in the [tools guide](docs/langchain/tools).
...
<Note>
  ...
  <Accordion title="Example: Using artifact for retrieval metadata">
    ...
    See the [RAG tutorial](docs/langchain/rag) for an end-to-end example of building retrieval [agents](docs/langchain/agents) with LangChain.
...
See the [integrations guides](docs/integrations/providers/overview) to get started with the
...
const model = await initChatModel('gpt-5-nano', { outputVersion: 'v1' })
```
...
<AccordionGroup>
  ...
  <Accordion title="Provider-Specific Blocks" icon="plug">
    ...
    Additional provider-specific content types may be found within the [reference documentation](docs/integrations/providers/overview) of each model provider.
...
[Chat models](docs/langchain/models) accept a sequence of message objects as input and return an [`AIMessage`](https://reference.langchain.com/javascript/classes/_langchain_core.messages.AIMessage.html) as output. Interactions are often stateless, so that a simple conversational loop involves invoking a model with a growing list of messages.

Refer to the below guides to learn more:

- Built-in features for [persisting and managing conversation histories](docs/langchain/short-term-memory)
- Strategies for managing context windows, including [trimming and summarizing messages](docs/langchain/short-term-memory#common-patterns)
