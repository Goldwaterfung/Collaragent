...
Models are the reasoning engine of [agents](docs/langchain/agents.md). They drive the agent's decision-making process, determining which tools to call, how to interpret results, and when to provide a final answer.
...
<Info>
  For provider-specific integration information and capabilities, see the provider's [chat model page](docs/integrations/chat).
</Info>
...
1. **With agents** - Models can be dynamically specified when creating an [agent](docs/langchain/agents#model).
...
The easiest way to get started with a standalone model in LangChain is to use `initChatModel` to initialize one from a [chat model provider](docs/integrations/chat) of your choice (examples below):

<Tabs>
  <Tab title="OpenAI">
    👉 Read the [OpenAI chat model integration docs](docs/integrations/chat/openai/)
  ...
  <Tab title="Anthropic">
    👉 Read the [Anthropic chat model integration docs](docs/integrations/chat/anthropic/)
  ...
  <Tab title="Azure">
    👉 Read the [Azure chat model integration docs](docs/integrations/chat/azure/)
...
See the [messages](docs/langchain/messages.md) guide for more detail on roles, types, and content.
...
<Accordion title="Advanced streaming topics">
  ...
  <Accordion title="&#x22;Auto-streaming&#x22; chat models">
    ...
    In [LangGraph agents](docs/langchain/agents.md), for example, you can call `model.invoke()` within nodes, but LangChain will automatically delegate to streaming if running in a streaming mode.
...
<Tip>
  See the [tools guide](docs/langchain/tools.md) for details and other options for creating tools.
</Tip>
...
When binding user-defined tools, the model's response includes a **request** to execute a tool. When using a model separately from an [agent](docs/langchain/agents.md), it is up to you to execute the requested tool and return the result back to the model for use in subsequent reasoning. When using an [agent](docs/langchain/agents.md), the agent loop will handle the tool execution loop for you.

Below, we show some common ways you can use tool calling.

<AccordionGroup>
  <Accordion title="Tool execution loop" icon="arrow-rotate-right">
    When a model returns tool calls, you need to execute the tools and pass the results back to the model. This creates a conversation loop where the model can use tool results to generate its final response. LangChain includes [agent](docs/langchain/agents.md) abstractions that handle this orchestration for you.
...
<Tip>
  To learn about structured output, see [Structured output](docs/langchain/structured-output.md).
</Tip>
...
Certain models can process and return non-textual data such as images, audio, and video. You can pass non-textual data to a model by providing [content blocks](docs/langchain/messages#message-content).

<Tip>
  All LangChain chat models with underlying multimodal capabilities support:

  1. Data in the cross-provider standard format (see [our messages guide](docs/langchain/messages.md))
...
See the [multimodal section](docs/langchain/messages#multimodal) of the messages guide for details.
...
If a model invokes a tool server-side, the content of the response message will include content representing the invocation and result of the tool. Accessing the [content blocks](docs/langchain/messages#standard-content-blocks) of the response will return the server-side tool calls and results in a provider-agnostic format:
