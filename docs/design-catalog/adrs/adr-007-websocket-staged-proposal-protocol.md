...
4. **Thread-Scoped Human Review Resolution**:
   - **Accept (`accept-changes`)**: Clears the proposal buffer for the targeted `threadId` (or all if omitted); changes remain committed in memory and are written to disk via single-flight `SerializedDrainQueue` persistence (superseding legacy 500ms `debouncedSave`, see [ADR-012](file:///Users/goldenfung/Documents/collaragent/docs/design-catalog/adrs/adr-012-event-driven-serialized-drain-system.md)).
