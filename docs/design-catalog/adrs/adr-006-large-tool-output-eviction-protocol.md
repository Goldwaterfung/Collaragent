...
Verified via `src/collaragent/middleware/fs.ts`, `src/collaragent/backends/FilesystemBackend.ts`, and `SqliteCheckpointStore.ts` (persisting evicted payloads into the relational `large_tool_outputs` table).
