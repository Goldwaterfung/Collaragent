...
CollarAgent manages complex desktop workspaces involving continuous real-time state synchronization, single-file SQLite database transactions (`.cagent`), legacy archive migration ETL pipelines, LangGraph agent checkpoint serialization, and full Lexical document persistence. Running database operations, JSON parsing, disk I/O, and Express REST endpoints directly inside the Electron Main process caused event loop stuttering and dropped frames in the Chromium UI thread.
...
- Heavy database disk I/O, SQLite WAL truncation, legacy archive migration ETL, and MessagePack BLOB serialization run off the main event loop, maintaining 60 FPS in the renderer.
