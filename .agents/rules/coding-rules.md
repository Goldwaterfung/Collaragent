...
1. **Never persist derived/catalog state into user settings.** Persisted configuration stores _user intent only_ (enabled flags, credential refs, explicit overrides). Values derivable from an upstream catalog — wire protocol (`api`), `thinkingFormat`, default model lists, builtin base URLs — MUST NOT be copied into settings files. If upstream changes, persisted snapshots silently rot.
